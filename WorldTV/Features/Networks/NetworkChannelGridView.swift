...
struct NetworkChannelGridView: View {
    @State private var viewModel: NetworkChannelGridViewModel
    private let favoritesStore: FavoritesStore

    init(
        broadcasterID: String,
        loadChannels: LoadChannelsByBroadcasterUseCase,
        favoritesStore: FavoritesStore
    ) {
        _viewModel = State(
            initialValue: NetworkChannelGridViewModel(
    ...
    var body: some View {
        Group {
            ...
            case .loaded(let content):
                channelGrid(title: content.broadcaster.name)
            case .empty:
    ...
    private func channelGrid(title: String) -> some View {
        ScrollView {
            LazyVStack(alignment: .leading, spacing: DesignTokens.sectionSpacing) {
                TVScreenHeader(verbatim: title, systemImage: "building.2")
